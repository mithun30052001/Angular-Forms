...
export class FormComponent implements OnInit{
  ...
  ngOnInit(): void {
    this.form = this.fb.group({
      phone: [''] // Initialize the phone form control
    });
  }

  onSubmit(): void {
  // Access the value of the phone form control using optional chaining
  const phoneNumber = this.form.get('phone')?.value;

  if (phoneNumber) {
    console.log('Phone number:', phoneNumber);

    // Perform any necessary actions with the phone number (e.g., send it to a backend service)
  } else {
    console.error('Phone form control value is null or undefined.');
  }
}
